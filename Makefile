...
checkStyle: tmp tmp/cpplint.py	
	find . -type f \( -name "*.cpp" -o -name "*.h" \) | grep -v "unit_tests/gtest-" | xargs python tmp/cpplint.py $(LINTER_PARAMS)

cppcheck:
	find . -type f \( -name "*.cpp" -o -name "*.h" \) | grep -v "unit_tests/gtest-" | xargs cppcheck --enable=all --inconclusive --std=c++11
