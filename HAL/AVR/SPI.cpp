...
uint8_t SPI_t::shift(const uint8_t data) {
    SPDR = data;
    while (!(SPSR & (1 << SPIF))) {
    }
    return SPDR;
...
SPI_Device_t::SPI_Device_t(const uint8_t cs_pin) {
    this->init(cs_pin);
}
...
uint8_t SPI_Device_t::shift(const uint8_t data) {
    return SPI.shift(data);
}

uint8_t SPI_Device_t::data_transfer(const uint8_t data) {
    this->enable();
