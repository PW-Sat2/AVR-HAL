...
struct __is_destructible
{
  private:
    ...
    template <class _Up>
    static typename remove_reference<decltype( declval<_Up>().~_Up(), declval<true_type>() )>::type __test(int);
...
template <size_t L, size_t Align = __alignof__(typename detail::__aligned_aux<L>::__type)>
